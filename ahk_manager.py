...
# Copied into the generated script's config folder at generate time: the script
# is run by AutoHotkey.exe on its own, so it cannot reach an icon bundled inside
# our exe.
AHK_ICON_NAME = "TextExpansionManager.ico"
# The app's own files -- the library, the settings, and the icon copy -- live in
# this folder beside whatever they belong to, rather than loose next to the
# executable. The script looks for its icon here first and then beside itself,
# so a lone .ahk carried to another machine with the icon next to it still
# finds one.
AHK_CONFIG_DIR_NAME = "config"
...
def _atomic_write_text(path: Path, text: str) -> None:
    ...
    try:
        # The app's files live in a config folder that a fresh install has yet
        # to create, and the temporary file lands in it too.
...
def _install_prompt_icon(script_path: Path, icon_source: Path | None) -> None:
    """Place the app icon in the script's config folder, for the prompts to load.
    ...
    try:
        if not icon_source.is_file():
            return
        target = script_path.parent / AHK_CONFIG_DIR_NAME / AHK_ICON_NAME
        if not target.exists() or target.stat().st_mtime < icon_source.stat().st_mtime:
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(icon_source, target)
        _remove_superseded_icon(script_path, icon_source)
    except OSError:
        return


def _remove_superseded_icon(script_path: Path, icon_source: Path) -> None:
    """Clear away the copy older builds left loose beside the script.

    Only when it is byte for byte the icon this app installs. Anything else is
    something the user put there, which the script still falls back to.
    """
    stale = script_path.parent / AHK_ICON_NAME
    try:
        if stale.is_file() and filecmp.cmp(stale, icon_source, shallow=False):
            stale.unlink()
    except OSError:
        return
...
def render_ahk(store: ExpansionStore, theme: str = DEFAULT_THEME) -> str:
    ...
    lines = [
        ...
        "",
        "TEM_TrayIcon := TEM_IconPath()",
        "if TEM_TrayIcon",
        "    TraySetIcon(TEM_TrayIcon)",
        "",
        *_icon_path_helper_lines(),
        "",
    ]
...
def _icon_path_helper_lines() -> list[str]:
    """Find the icon: the config folder first, then beside the script.

    Both are relative to the script rather than to the app, so a script
    generated into some other folder -- or carried to a machine with no app on
    it -- looks where it actually sits. Neither present means the default
    AutoHotkey icon, never an error.
    """
    return [
        "TEM_IconPath() {",
        f'    inConfig := A_ScriptDir "\\{AHK_CONFIG_DIR_NAME}\\{AHK_ICON_NAME}"',
        "    if FileExist(inConfig)",
        "        return inConfig",
        f'    beside := A_ScriptDir "\\{AHK_ICON_NAME}"',
        "    if FileExist(beside)",
        "        return beside",
        '    return ""',
        "}",
